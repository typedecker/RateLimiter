...
class RateLimiter() :
        ...
        if func_name in self.requests :
            ...
            if func_info['requests'] >= func_info['ratelimit'] :
                async def ratelimited_func(*args, **kwargs) :
                    self.requests[func_name]['pending_requests'].append({'type': 'func_call', 'args': args, 'kwargs': kwargs})
                    return
            else :
                async def ratelimited_func(*args, **kwargs) :
    ...
    async def free(self, func) :
        func_name = func.__name__
        if func_name in self.requests :
            self.requests[func_name]['pending_requests'].append({'type': 'free', 'args': [], 'kwargs': {}})
            return True
        return False
